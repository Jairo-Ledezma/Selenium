...
public class MouseAndKeyboardActions {
	...
	public static void main(String[] args) {
		...
		WebElement navIcon = driver.findElement(By.cssSelector("[id=\"nav-link-accountList\"]")); // locates the element
																									// we will hover on

		Actions a = new Actions(driver);// instance of the action class, it receives the driver as parameter

		a.moveToElement(navIcon).perform(); // the instance of the action class (a) using the method moveToElement()
											// which received the element we will hover on

		WebElement searchBar = driver.findElement(By.cssSelector("#twotabsearchtextbox"));

		a.moveToElement(searchBar).click().keyDown(Keys.SHIFT).sendKeys("test").doubleClick().perform(); // moves to
																											// element,
																											// clicks on
																											// it,
																											// keyDown()
