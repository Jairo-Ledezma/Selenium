...
public class EcommerceSite {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		List<WebElement> products = driver.findElements(By.xpath("//h4[@class='product-name']")); // creates a list of strings with the name of the products, note that name of the products structure is name - 1Kg
		...
		for (int i = 0; i < products.size(); i++) { // initiates a loop that traverses the list of products on the website
			...
			if (itemsNeededList.contains(actualName)) { // conditional testing the name of the product on the website (already trimmed) is contained in the arrayList of items needed
				j++;// increments the count so we can break the loop once all the items in the array list are selected
				//driver.findElements(By.xpath("//div[@class='product-action'] //button[@type = 'button']")).get(i).click();
				driver.findElements(By.xpath("//div[@class='product-action'] /button")).get(i).click(); // accesses the specific webelement "add to cart button" (notice the get(i) so we click on the element that is being looped trough) and clicks on it
				...
				if (j == itemsNeededList.size()) { // conditional to test that all the elements in the arraylist of needed items are added based on the size of the arraylist 
					break; // if the elements clicked matches the number of indexes in the array list of items needed it breaks the loop to avoid additional looping
				}
			}
		}
