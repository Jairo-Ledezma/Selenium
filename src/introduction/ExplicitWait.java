...
public class ExplicitWait {
	...
	public static void main(String[] args) throws InterruptedException {
		// Explicit way targets specific elements to wait for an amount of time before throwing an exception 
		// scenarios in which loading time is lengthy Explicit way is used to run the script 
		//Note that it does not have an effect on every element or step of the script as implicit wait do
	...
	public static void checkoutProcess(WebDriver driver ) {
		WebDriverWait w = new WebDriverWait(driver, Duration.ofSeconds(5));// explicit wait declaration, creates an object of WebDriverwait class that receives the driver and Duration.ofSeconds() parameters
		...
		w.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("input.promoCode"))); // w(instance of wait) . until(method of the instance) takes as parameter ExpectedConditions class that has many methods to wait 
		...
		promoBtn.click();
		w.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("span.promoInfo")));// w(instance of wait) . until(method of the instance) takes as parameter ExpectedConditions class that has many methods to wait 
