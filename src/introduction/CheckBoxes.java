...
public class CheckBoxes {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Assert.assertTrue(isSelected); // assertion from testng library, this will stop the execution if the boolean evaluates to false since it expects true
		...
		Thread.sleep(5000l);
		Assert.assertFalse(isSelected);// assertion from testng library, this will stop the execution if the boolean evaluates to true since it expects false
		Assert.assertTrue(isSelected2);
		
		
		//checkbox count
		List <WebElement> checkBoxCount = driver.findElements(By.cssSelector("input[type='checkbox']")); // gets all the elements with type checkbox and input tag and puts them in a list
